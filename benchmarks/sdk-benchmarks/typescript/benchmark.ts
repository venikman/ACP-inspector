...
function makeTokenUpdate(tokenCount: number): string {
...
function runThroughput(count: number): void {
...
// Benchmark: Codec
function runCodec(count: number): void {
  const messages = [initializeRequest, sessionNewRequest, sessionUpdateNotification, promptRequest];
...
// Benchmark: Tokens
function runTokens(count: number, tokensPerMsg: number): void {
...
// Parse arguments
function parseArgs(): { mode: string; count: number; tokens: number } {

...
**Scope**: `protocol/src/`, `runtime/src/`, `sentinel/src/`, `sentinel/tests/`
...
```bash
dotnet build ACP-inspector.slnx -warnaserror
...
| ------ | ----- | -------- | ----- |
| `runtime/src/Acp.Codec.fs` | 3130 | 🔴 HIGH | Needs splitting into multiple modules |
| `runtime/src/Acp.Connection.fs` | 884 | 🟡 MEDIUM | Consider extracting state machine |
| `protocol/src/Acp.Domain.fs` | 757 | 🟢 LOW | Domain types are cohesive |
| `sentinel/src/Acp.Validation.fs` | 704 | 🟢 LOW | Validation logic is cohesive |
| `sentinel/src/Acp.FSharpTokenizer.fs` | 528 | 🟢 LOW | Self-contained tokenizer |
| `sentinel/tests/Acp.SemanticTests.fs` | 537 | 🟢 LOW | Test file, acceptable |
...
1. `sentinel/src/Acp.Assurance.fs` ← New BC-001 module
2. `sentinel/src/Acp.Capability.fs` ← New BC-003 module
3. `sentinel/src/Acp.Semantic.fs` ← New BC-002 module
4. `sentinel/tests/Acp.AssuranceTests.fs` ← New tests
5. `sentinel/tests/Acp.CapabilityTests.fs` ← New tests
6. `sentinel/tests/Acp.SemanticTests.fs` ← New tests

**Action**: Run `dotnet fantomas protocol/src runtime/src sentinel/src sentinel/tests --write` to auto-fix.
...
| File | Line | Content |
| ------ | ---- | ------- |
| `sentinel/tests/Epistemology.Harness/EvalTests.fs` | 10 | `// TODO: Implement tests for epistemic reasoning` |
| `sentinel/tests/Epistemology.Harness/ValidationTaxonomyTests.fs` | 10 | `// TODO: Implement actual tests` |

### Stub Tests (Assert.True(true))

Both files in `sentinel/tests/Epistemology.Harness/` contain placeholder tests:
...
- `sentinel/src/Acp.Assurance.fs` - Well documented
- `sentinel/src/Acp.Capability.fs` - Well documented
- `sentinel/src/Acp.Semantic.fs` - Well documented
- `runtime/src/Acp.Codec.fs` - Module-level docs
...
1. **Format all files**
   ...
   ```bash
   dotnet fantomas protocol/src runtime/src sentinel/src sentinel/tests --write
   ```
...
```text
Source Lines:    ~9,500 (protocol/src + runtime/src + sentinel/src)
Test Lines:      ~5,800 (sentinel/tests/)

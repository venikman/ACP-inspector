...
module Codec =
    ...
    // -----------------
    // ACP type decoders/encoders
    // -----------------

    module private AcpJson =
        ...
        let private decodeLoadSessionResponseModes
            (nodeOpt: JsonNode option)
            : Result<SessionModeState option, string> =
            match nodeOpt with
            | None -> Ok None
            | Some node ->
                result {
        ...
        let private decodeAvailableCommandInput
            (nodeOpt: JsonNode option)
            : Result<AvailableCommandInput option, string> =
            match nodeOpt with
            | None -> Ok None
            | Some node ->
        ...
        let decodeAgentResult
            ...
            | PendingClientRequest.ExtRequest methodName ->
                Ok(AgentToClientMessage.ExtResponse(methodName, resultNodeOpt))
        ...
        let decodeClientResult
            ...
            | PendingAgentRequest.ExtRequest methodName ->
                Ok(ClientToAgentMessage.ExtResponse(methodName, resultNodeOpt))
        ...
        let encodeClientMessage
            (idOpt: RequestId option)
            (msg: ClientToAgentMessage)
            : Result<JsonObject, EncodeError> =
            let o = JsonObject()
    ...
    let decode (direction: Direction) (state: CodecState) (json: string) : Result<CodecState * Message, DecodeError> =
        ...
        let decodeCore (node: JsonNode | null) : Result<CodecState * Message, DecodeError> =
            ...
            | :? JsonObject as o ->
                // jsonrpc must be "2.0"
                match Json.tryGet "jsonrpc" o with
                | None -> Error(DecodeError.InvalidJsonRpc "missing 'jsonrpc'")
                | Some v ->
                    match Json.asString v with
                    | Error e -> Error(DecodeError.InvalidJsonRpc e)
                    | Ok ver when ver <> "2.0" -> Error(DecodeError.InvalidJsonRpc "jsonrpc must be '2.0'")
                    | Ok _ ->
                        let hasMethod = Json.tryGetAllowNull "method" o |> Option.isSome
                        let hasResult = Json.tryGetAllowNull "result" o |> Option.isSome
                        let hasError = Json.tryGetAllowNull "error" o |> Option.isSome

                        // request/notification
                        if hasMethod then
                            let methodName =
                                Json.tryGet "method" o
                                |> Option.bind (fun n -> Json.asString n |> Result.toOption)
                                |> Option.defaultValue ""

                            if String.IsNullOrWhiteSpace methodName then
                                Error(DecodeError.InvalidJsonRpc "method must be a non-empty string")
                            else
                                let paramsNodeOpt = Json.tryGet "params" o |> Json.cloneOpt
                                let idNodeOpt = Json.tryGet "id" o

                                match Json.decodeRequestId idNodeOpt with
                                | Error e -> Error(DecodeError.InvalidJsonRpc e)
                                | Ok None ->
                                    // Notification
                                    match direction with
                                    | Direction.FromClient ->
                                        match AcpJson.decodeClientNotification methodName paramsNodeOpt with
                                        | Ok msg -> Ok(state, Message.FromClient msg)
                                        | Error e -> Error(DecodeError.InvalidParams(methodName, e))
                                    | Direction.FromAgent ->
                                        match AcpJson.decodeAgentNotification methodName paramsNodeOpt with
                                ...
                                | Ok(Some id) ->
                                    ...
                                    match direction with
                                    | Direction.FromClient ->
                                        match AcpJson.decodeClientRequest methodName paramsNodeOpt with
                                    ...
                                    | Direction.FromAgent ->
                                        match AcpJson.decodeAgentRequest methodName paramsNodeOpt with
                            ...
                            | Ok(Some id) ->
                                ...
                                | Direction.FromAgent ->
                                    ...
                                    | Some pending ->
                                        ...
                                        if hasError then
                                            match Json.tryGet "error" o with
                                            | None ->
                                                Error(
                                                    DecodeError.InvalidJsonRpc "response has error flag but no 'error'"
                                                )
                                            | Some errNode ->
                                                match Json.decodeError errNode with
                                                | Error e -> Error(DecodeError.InvalidError e)
                                                | Ok err ->
                                                    let msg = AcpJson.decodeAgentError pending err
                                                    Ok(next, Message.FromAgent msg)
                                        else
                                            let resultNodeOpt = Json.tryGet "result" o |> Json.cloneOpt

                                            match AcpJson.decodeAgentResult pending resultNodeOpt with
                                            | Ok msg -> Ok(next, Message.FromAgent msg)
                                            | Error e ->
                                                Error(
                                                    DecodeError.InvalidResult(AcpJson.methodOfPendingClient pending, e)
                                                )
                                    ...
                                    | Some pending ->
                                        ...
                                        if hasError then
                                            ...
                                            | Some errNode ->
                                                ...
                                                | Error e -> Error(DecodeError.InvalidError e)
                                                | Ok err ->
                                                    let msg = AcpJson.decodeClientError pending err
                                                    Ok(next, Message.FromClient msg)
                                        else
                                            let resultNodeOpt = Json.tryGet "result" o |> Json.cloneOpt

                                            match AcpJson.decodeClientResult pending resultNodeOpt with
                                            | Ok msg -> Ok(next, Message.FromClient msg)
                                            | Error e ->
                                                Error(
                                                    DecodeError.InvalidResult(AcpJson.methodOfPendingAgent pending, e)
                                                )
    ...
    let encode (idOpt: RequestId option) (msg: Message) : Result<string, EncodeError> =
        let objResult =
            match msg with
            | Message.FromClient c -> AcpJson.encodeClientMessage idOpt c
            | Message.FromAgent a -> AcpJson.encodeAgentMessage idOpt a

...
module Validation =
    ...
    module SessionTrace =
        ...
        let append (msg : Message) (trace : SessionTrace) =
            { trace with messages = trace.messages @ [ msg ] }

        let length (trace : SessionTrace) =
            trace.messages.Length

    module FromProtocol =

        let private sessionFromMessage = function
        ...
        let private subjectOfError
            ...
            (msg   : Message)
            (error : ProtocolError)
            (traceIndex : int option) : Subject * SessionId option * int option =
        ...
        let ofProtocolError
            ...
            (msg        : Message)
            (error      : ProtocolError)
            (traceIndex : int option) : ValidationFinding =
        ...
        |> List.iteri (fun idx msg ->
            ...
            | Message.FromClient (ClientToAgentMessage.SessionPrompt p) when p.sessionId = trace.sessionId ->
                ...
                if openPrompts > 0 then
                    ...
                    findings <- findings @ [ { lane       = Lane.Session
                                               severity   = Severity.Error
                                               subject    = subject
                                               failure    = Some failure
                                               sessionId  = Some trace.sessionId
    ...
    module Transport =
        ...
        let validateSize
            ...
            (subject    : Subject)
            (traceIndex : int option)
            (actualBytes: int) : ValidationFinding option =
    ...
    module MetadataProfile =
        ...
        let validate
            ...
            (kind       : string)
            (subject    : Subject)
            (traceIndex : int option) : ValidationFinding option =
            ...
            | MetadataPolicy.AllowKinds kinds ->
                if kinds |> List.contains kind then None
                else
                    let failure =
    ...
    let runWithValidation
            ...
            | Some rp ->
                ...
                |> List.iter (function
                    | Prompting.ContentBlock.Other (kind, _) ->
                        match MetadataProfile.validate policy kind (Subject.MessageAt (traceIndex, msg)) (Some traceIndex) with
            ...
            if continueProcessing then
                ...
                | Ok phase ->
                    ...
                    | Error e ->
                        ...
                        phaseResult <- Error e
                        if stopOnFirstError then continueProcessing <- false
                | Error _ ->
        ...
        { finalPhase = phaseResult
          trace      = trace
          findings   = findings @ sessionCancelFindings @ sessionConcurrencyFindings }
    ...
    module PromptOutcome =
        ...
        let private tryLastStopReason sessionId (trace : SessionTrace) =
            ...
            |> List.choose (function
                | Message.FromAgent (AgentToClientMessage.SessionPromptResult r) when r.sessionId = sessionId -> Some r.stopReason
                | _ -> None)
            |> List.tryLast
        ...
        let classify
            ...
            (trace      : SessionTrace)
            (finalPhase : Result<Phase, ProtocolError>)
            (findings   : ValidationFinding list) : PromptTurnOutcome =
            ...
            | Ok _ ->
                ...
                | None ->
                    ...
                    | Some f ->
                        let msg = f.failure |> Option.map (fun v -> v.message) |> Option.defaultValue "prompt turn failed without stop reason"
                        PromptTurnOutcome.DomainError (DomainErrorOutcome.ProtocolViolation (f.failure.Value.code, msg))
                    | None ->
                        PromptTurnOutcome.DomainError (DomainErrorOutcome.AgentInternalFailure (sprintf "No SessionPromptResult observed for session %s" (SessionId.value sessionId)))

...
module ValidationTests =
    ...
    let private mcpCaps : McpCapabilities =
        { http = false
          sse  = false }
    ...
    let mkHappyTrace (sid : SessionId) : Message list =
        [ Message.FromClient (ClientToAgentMessage.Initialize initParams)
          ...
          Message.FromAgent  (AgentToClientMessage.SessionPromptResult { sessionId = sid; stopReason = StopReason.EndTurn }) ]
    ...
    let mkSequentialPromptsTraceGood (sid : SessionId) : Message list =
        [ Message.FromClient (ClientToAgentMessage.Initialize initParams)
          ...
          Message.FromClient (ClientToAgentMessage.SessionNew { cwd = "."; mcpServers = [] })
          Message.FromAgent  (AgentToClientMessage.SessionNewResult { sessionId = sid })
          Message.FromClient (ClientToAgentMessage.SessionPrompt { sessionId = sid; content = [ ContentBlock.Text "p1" ] })
          Message.FromAgent  (AgentToClientMessage.SessionPromptResult { sessionId = sid; stopReason = StopReason.EndTurn })
          Message.FromClient (ClientToAgentMessage.SessionPrompt { sessionId = sid; content = [ ContentBlock.Text "p2" ] })
          Message.FromAgent  (AgentToClientMessage.SessionPromptResult { sessionId = sid; stopReason = StopReason.EndTurn }) ]
    ...
    let ``sequential prompts yield no Session-lane concurrency errors`` () =
        ...
        let sessionFindings =
            ...
            |> List.filter (fun f ->
                ...
                match f.failure with
                | Some failure ->
                    failure.code = "ACP.SESSION.MULTIPLE_PROMPTS_IN_FLIGHT" || failure.code = "ACP.SESSION.RESULT_WITHOUT_PROMPT"
                | None -> false)
    ...
    let ``transport size violation produces Transport lane finding`` () =
        let profile : RuntimeProfile =
            { metadata  = MetadataPolicy.AllowOpaque
              transport = Some { lineSeparator = None; maxFrameBytes = None; maxMessageBytes = Some 10; metaEnvelope = None } }
    ...
    let ``metadata disallow policy flags Other kind`` () =
        let findingOpt = validate MetadataPolicy.Disallow "image/png" Subject.Connection (Some 1)
        match findingOpt with
        | Some f ->
            Assert.Equal(Lane.Transport, f.lane)
            Assert.Equal(Severity.Error, f.severity)

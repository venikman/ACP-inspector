...
module EvalTests =
    ...
    let ``non-empty text passes code judge`` () =
        let msg = Message.FromClient (ClientToAgentMessage.SessionPrompt { sessionId = sid; content = [ ContentBlock.Text "hi" ] })
        let findings = runPromptChecks defaultProfile msg
        Assert.True(findings.IsEmpty)

    [<Fact>]
    let ``empty prompt fails code judge`` () =
        let msg = Message.FromClient (ClientToAgentMessage.SessionPrompt { sessionId = sid; content = [] })
        let findings = runPromptChecks defaultProfile msg
    ...
    let ``tool call with text passes`` () =
        ...
        let msg =
            Message.FromAgent (AgentToClientMessage.SessionUpdate { sessionId = sid; update = SessionUpdate.ToolCall tc })

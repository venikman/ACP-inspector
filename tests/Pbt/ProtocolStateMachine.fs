...
module ProtocolStateMachine =
    ...
    let private opInitialize (p : InitializeParams) =
        StateMachine.operation "Initialize"
            (fun _ -> WaitingForInitializeResult)
            (fun (actual, model) ->
    ...
    let private opInitializeResult (r : InitializeResult) =
        StateMachine.operation "InitializeResult"
            (fun _ -> Ready Map.empty)
            (fun (actual, model) ->
    ...
    let private opSessionNewReq =
        StateMachine.operation "SessionNew"
            (fun m -> m)
            (fun (actual, model) ->
                let msg =
    ...
    let private opSessionNewResult (sid : SessionId) =
        StateMachine.operation "SessionNewResult"
            (function Ready sessions -> Ready (sessions |> Map.add sid false) | m -> m)
            (fun (actual, model) ->
                let msg =
    ...
    let private opSessionLoadReq (sid : SessionId) =
        StateMachine.operation "SessionLoad"
            (fun m -> m)
            (fun (actual, model) ->
                let msg =
                    Message.FromClient (ClientToAgentMessage.SessionLoad { sessionId = sid; cwd = "."; mcpServers = [] })
                apply actual msg && (projectPhase actual.Value = model))

    let private opSessionLoadResult (sid : SessionId) =
        StateMachine.operation "SessionLoadResult"
            (function
             | Ready sessions ->
                if sessions |> Map.containsKey sid then Ready sessions
                else Ready (sessions |> Map.add sid false)
             | m -> m)
            (fun (actual, model) ->
                let msg =
                ...
                | Ready sessions ->
                    let sessions' =
                        if sessions |> Map.containsKey sid then sessions
                        else sessions |> Map.add sid false
                    projectPhase actual.Value = Ready sessions'
                | _ -> false)

    let private opPromptReq (sid : SessionId) =
        StateMachine.operation "SessionPrompt"
            (function Ready sessions -> Ready (sessions |> Map.add sid true) | m -> m)
            (fun (actual, model) ->
                let msg =
    ...
    let private opPromptResult (sid : SessionId) (sr : StopReason) =
        StateMachine.operation "SessionPromptResult"
            (function Ready sessions -> Ready (sessions |> Map.add sid false) | m -> m)
            (fun (actual, model) ->
                let msg =
    ...
    let private opCancelReq (sid : SessionId) =
        StateMachine.operation "SessionCancel"
            (fun m -> m)
            (fun (actual, model) ->
                let msg =
    ...
    let private opSessionUpdate (sid : SessionId) =
        StateMachine.operation "SessionUpdate"
            (fun m -> m)
            (fun (actual, model) ->
                let msg =
                    Message.FromAgent (
    ...
    let private opRequestPermission (sid : SessionId) =
        ...
        StateMachine.operation "RequestPermission"
            (fun m -> m)
            (fun (actual, model) ->
                let msg =
                    Message.FromAgent (
    ...
    let private setup =
        StateMachine.setup
            (fun () -> ref Phase.AwaitingInitialize)
            (fun () -> AwaitingInitialize)

    let private next (model : Model) : Gen<Operation<Phase ref, Model>> =
        match model with
        | AwaitingInitialize ->
            Generators.genInitializeParams |> G.map opInitialize

        | WaitingForInitializeResult ->
            Generators.genInitializeResult |> G.map opInitializeResult

        | Ready sessions ->
            let known = sessions |> Map.toList |> List.map fst
            let idle = sessions |> Map.toList |> List.choose (fun (sid, inflight) -> if inflight then None else Some sid)
            let inflight = sessions |> Map.toList |> List.choose (fun (sid, inflight) -> if inflight then Some sid else None)
            ...
            let genLoadReq =
                Generators.genSessionId |> G.map opSessionLoadReq

            let genLoadResult =
                Generators.genSessionId |> G.map opSessionLoadResult

            let genPromptReq =
                G.elements idle |> G.map opPromptReq

            let genPromptResult =
                G.elements inflight
                |> G.bind (fun sid ->
                    Generators.genStopReason |> G.map (fun sr -> opPromptResult sid sr))

            let genCancelReq =
                G.elements inflight |> G.map opCancelReq

            let genUpdate =
                G.elements known |> G.map opSessionUpdate

            let genReqPerm =
                G.elements inflight |> G.map opRequestPermission
            ...
            let choices : (int * Gen<Operation<Phase ref, Model>>) list =
                [ 2, G.constant opSessionNewReq
                  ...
                  1, genLoadReq
                  1, genLoadResult
                  if idle.Length > 0 then 3, genPromptReq else 0, G.constant opSessionNewReq
                  if inflight.Length > 0 then 3, genPromptResult else 0, G.constant opSessionNewReq
                  if inflight.Length > 0 then 1, genCancelReq else 0, G.constant opSessionNewReq
                  if known.Length > 0 then 1, genUpdate else 0, G.constant opSessionNewReq
                  if inflight.Length > 0 then 1, genReqPerm else 0, G.constant opSessionNewReq ]

...
module RuntimeAdapterTests =
    ...
    let fsCaps =
        { FileSystemCapabilities.readTextFile = true
          writeTextFile = true }

    let clientCaps =
        { ClientCapabilities.fs = fsCaps
          terminal = false }
    ...
    let agentInfo =
        { ImplementationInfo.name = "test-agent"
          title = None
          version = "0.0.0-test" }
    ...
    let ``custom eval profile allows empty prompt`` () =
        ...
        let evalFindings = result.findings |> List.filter (fun f -> f.lane = Lane.Eval)

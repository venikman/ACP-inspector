...
Use the $ce-triage skill for this command and follow its instructions.

- First set the /prompts:model to Haiku
...
2. Start work on approved items:
   ...
   ```bash
   /prompts:ce-resolve_todo_parallel  # Work on multiple approved items efficiently
   ```
...
- ❌ That's for /prompts:ce-resolve_todo_parallel phase
```
...
1. run /prompts:ce-resolve_todo_parallel to resolve the todos
2. commit the todos
3. nothing, go chill
```

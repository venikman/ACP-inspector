...
Use the $ce-deepen-plan skill for this command and follow its instructions.
...
This command takes an existing plan (from `/prompts:ce-workflows-plan`) and enhances each section with parallel research agents. Each major element gets its own dedicated research sub-agent to find:
...
<thinking>
Check for documented learnings from /prompts:ce-workflows-compound. These are solved problems stored as markdown files. Spawn a sub-agent for each learning to check if it's relevant.
</thinking>

**LEARNINGS LOCATION - Check these exact folders:**

```
docs/solutions/           <-- PRIMARY: Project-level learnings (created by /prompts:ce-workflows-compound)
├── performance-issues/
│   └── *.md
├── debugging-patterns/
│   └── *.md
...
2. **Learnings/Solutions sub-agents** - Relevant documented learnings from /prompts:ce-workflows-compound
...
2. **Run `/prompts:ce-plan_review`** - Get feedback from reviewers on enhanced plan
3. **Start `/prompts:ce-workflows-work`** - Begin implementing this enhanced plan
...
**Before (from /prompts:ce-workflows-plan):**
```markdown
## Technical Approach
